...
from korean_to_english_evaluator import evaluate_korean_name, create_ko_to_en_evaluator
from english_to_korean_evaluator import evaluate_english_name, create_en_to_ko_evaluator
from korean_name_evaluator import generate_html_report
from terminologists_manual_links import get_resources_for_direction, get_verification_process_text
...
def verify_real_person_name(
    ...
    if direction == "KO-EN":
        ...
        recommendations.append("For real person names, hyphenation is mandatory between syllables")
        recommendations.append("Verify with multiple official sources including NIKL standards")
        if "teamwork_verification" in result and result["teamwork_verification"]:
            recommendations.append("Follow existing verified translations in Teamwork for consistency")
        ...
        result["real_person_rules"] = {
            ...
            "requires_kyonshik_confirmation": "netflix" in result.get("context", "").lower(),
        ...
        # Add stricter recommendations for real person names
        recommendations = result.get("recommendations", [])
        recommendations.append("For real person names created after 08/11/2020, confirm notation through Kyonshik for NF tasks")
        recommendations.append("For non-NF projects, confirm through Hazel (phonetician)")
        recommendations.append("Record 'HZ Original Notation' in the Korean target of the Termbase")
        result["recommendations"] = recommendations
...
def batch_verify_real_person_names(
    ...
    if results["ko_en_results"]:
        ...
        print(f"Korean to English real person verification report saved to: {ko_report_path}")
    ...
    results["verification_summary"] = {
        ...
        "ko_en_compliant": sum(1 for r in results["ko_en_results"] if r.get("compliant", False)),
        "en_ko_compliant": sum(1 for r in results["en_ko_results"] if r.get("compliant", False)),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-teamwork",
        action="store_true",
        help="Disable Teamwork verification"
    )
    ...
    if summary["ko_en_count"] > 0:
        print(f"Korean names (KO-EN): {summary['ko_en_count']} verified, {summary['ko_en_compliant']} compliant")
    
    if summary["en_ko_count"] > 0:
        print(f"English names (EN-KO): {summary['en_ko_count']} verified, {summary['en_ko_compliant']} compliant")
