...
def main():
    ...
    avg_score = int(sum(r.get("overall_score", 0) for r in results) / len(results)) if results else 0
    
    print("\nEvaluation complete!")
    print(f"Evaluated {len(names)} names")
    print(f"Compliant: {compliant_count}, Non-compliant: {len(names) - compliant_count}")
