...
from terminologists_manual_links import get_verification_process_text, get_resources_for_direction
...
class KoToEnNameEvaluation(BaseModel):
    ...
    romanization_compliant: bool = Field(description="Whether the romanization follows NIKL rules")
    hyphenation_compliant: bool = Field(description="Whether the hyphenation follows CF guidelines")
    capitalization_compliant: bool = Field(description="Whether the capitalization follows CF guidelines")
    verification_process: Dict[str, int] = Field(default_factory=dict, description="Scores for each verification process step (0-100)")
    verification_sources: List[str] = Field(default_factory=list, description="Sources used for verification")
    reference_links: Optional[List[str]] = Field(default=None, description="Links to reference materials")
    termbase_entry: Optional[Dict[str, str]] = Field(default=None, description="Recommended Termbase entry details")
    compliant: bool = Field(description="Whether the English notation is compliant with guidelines")
    ...
    def name_evaluator(name, teamwork_results=None):
        ...
        if isinstance(teamwork_results, dict) and teamwork_results.get("matches"):
            ...
            for match in matches:
                ...
                teamwork_context += f"  Translation: {match.get('translation', 'Not specified')}\n"
        else:
        ...
        try:
            ...
            evaluation.update({
                ...
                "capitalization_compliant": data.get("capitalization_compliant", False),
            ...
            try:
                ...
                english_match = re.search(r'"english_notation":\s*"([^"]+)"', response_text)
                if english_match:
                    evaluation["english_notation"] = english_match.group(1).strip()
                    
                # Extract compliance and score
                compliant_match = re.search(r'"compliant":\s*(true|false)', response_text, re.IGNORECASE)
                if compliant_match:
...
def generate_termbase_entries(results: List[Dict], output_file: str = "reports/korean_to_english_termbase_entries.txt"):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="reports/korean_to_english_termbase_entries.txt", help="Output file path for termbase entries")
    parser.add_argument("--no-teamwork", action="store_true", help="Disable Teamwork verification")
