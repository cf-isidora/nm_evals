...
def process_names(
    ...
    results = {"ko_en_results": [], "en_ko_results": [], "teamwork_verification": []}

    # Log message about local resources mode
    if use_local_only:
        print(
            "Using local resources mode: Only resources from the data directory will be used."
        )
    ...
    if auto_detect:
        ...
        if categorized["ko"]:
            ...
            ko_results = evaluate_korean_names(
                categorized["ko"], check_teamwork=verify_in_teamwork
            )
            ...
            if (
                post_to_teamwork
                and os.environ.get("TEAMWORK_API_KEY")
                ...
                for result in ko_results:
                    ...
                    if success:
                        print(
                            f"✓ Posted evaluation for '{name}' to Teamwork: {message}"
                        )
                    else:
                        print(
                            f"✗ Failed to post evaluation for '{name}' to Teamwork: {message}"
                        )

        # Process English names (EN-KO direction)
        if categorized["en"]:
            print(f"Processing {len(categorized['en'])} English names (EN-KO)...")
            en_results = evaluate_english_names(
                categorized["en"], check_teamwork=verify_in_teamwork
            )
        ...
        if direction == "KO-EN":
            ...
            en_results = evaluate_english_names(
                names, check_teamwork=verify_in_teamwork
            )
            results["en_ko_results"] = en_results
        ...
        print(
            "Please specify a direction ('KO-EN' or 'EN-KO') or enable auto-detection."
        )
...
def parse_arguments():
    ...
    name_group.add_argument("--names", type=str, nargs="+", help="Names to evaluate")
    ...
    teamwork_group.add_argument(
        "--teamwork-project-id", type=str, help="Teamwork project ID to create tasks in"
    )
    teamwork_group.add_argument(
        "--no-teamwork",
        action="store_true",
...
def check_available_local_resources():
    ...
    local_resources = {
        ...
        "links": os.path.exists(
            os.path.join(DATA_DIRECTORY, "terminologists_manual_links.txt")
        ),
        "tm_depository": os.path.exists(
            os.path.join(DATA_DIRECTORY, "TM Training Landscape.xlsx")
        ),
        "terminology_depository": os.path.exists(
            os.path.join(DATA_DIRECTORY, "Terminologists' Depository.xlsx")
        ),
        "mmt": os.path.exists(
            os.path.join(
                DATA_DIRECTORY,
                "MAIN MARKETING TRANSLATIONS (MMT - Source of Truth).xlsx",
            )
        ),
        "tm_job_organizer": os.path.exists(
            os.path.join(DATA_DIRECTORY, "TM Job Organizer.xlsx")
        ),
        "task_tracker": os.path.exists(
            os.path.join(DATA_DIRECTORY, "Task Tracker.xlsx")
        ),
    }
...
def main():
    ...
    if ko_en_count > 0:
        ...
        print(
            f"Korean names (KO-EN): {ko_en_count} evaluated, {ko_compliant} compliant"
        )
    ...
    if en_ko_count > 0:
        ...
        print(
            f"English names (EN-KO): {en_ko_count} evaluated, {en_compliant} compliant"
        )
    ...
    if (args.verify_in_teamwork or args.post_to_teamwork) and not args.no_teamwork:
        ...
        if args.post_to_teamwork and not args.no_teamwork:
            ...
            if args.teamwork_project_id:
                print(
                    f"- View tasks in project: https://{teamwork_domain}.teamwork.com/app/projects/{args.teamwork_project_id}/tasks"
                )
